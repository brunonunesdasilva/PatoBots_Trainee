...
static void my_platform_on_controller_data(uni_hid_device_t* d, uni_controller_t* ctl) {
    ...
    int vel_direita, vel_esquerda;
 
    if (raw == 0 && direcao != 0) {
        // Sem aceleração/freio, mas com direção: gira no próprio eixo (pivot turn)
        int vel_giro = (direcao * 1023) / 512;
        vel_direita = -vel_giro;
        vel_esquerda = vel_giro;
    } else {
        // Cálculo arcade: mistura aceleração com direção (curva enquanto anda)
        int ajuste_curva = (direcao * raw) / 512;
        vel_direita = raw - ajuste_curva;
        vel_esquerda = raw + ajuste_curva;
    }
 
    // Limitação a -1023..1023
    if (vel_esquerda > 1023) vel_esquerda = 1023;
    if (vel_esquerda < -1023) vel_esquerda = -1023;
    if (vel_direita > 1023) vel_direita = 1023;
    if (vel_direita < -1023) vel_direita = -1023;
 
    // Deadzone: só zera se nem acelerando/freando nem girando
    if (aceleracao < 20 && freio > -20 && direcao == 0) {
        vel_direita = 0;
        vel_esquerda = 0;
    }
